...
| `/标准礼貌` | 超礼貌·标准 | 非常礼貌、正式、克制；对应预设为活人10/礼貌95 |
| `/活人感` | 超礼貌·活人感 | 非常礼貌但更像真人聊天：口语化、语气亲切，可使用颜文字；对应预设为活人80/礼貌85 |
| `/设置` | 参数设置 | 查看或设置活人值/礼貌值，例如 `/设置 活人70 礼貌85` |
...
- `/设置`：查看当前活人值/礼貌值。
- `/设置 活人70 礼貌85` 或 `/设置 活人=70 礼貌=85`：修改并保存。
- `/恢复默认`：重置为活人60/礼貌80。
- `/预设`：查看全部预设；`/预设 标准礼貌`、`/预设 活人感`、`/预设 闲聊`、`/预设 自动回复` 一键切换。
- `/记住 姓名=张三 班级=24跨传x班 学号=24xxxxxxx`：保存个人信息，之后起草自动带入，不用每次重问。
